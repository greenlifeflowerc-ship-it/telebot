...
def validate_config() -> None:
    ...
    if not WEBHOOK_SECRET:
        ...
        problems.append("WEBHOOK_SECRET invalid chars.")
...
def _refresh_proxy_pool() -> None:
    ...
    if PROXY_FETCH_URL:
        try:
            ...
            logger.warning("Failed to fetch proxies: %s", exc)
    with _proxy_lock:
        if new_proxies:
            _proxy_pool = list(set(new_proxies))  # deduplicate
            _last_proxy_refresh = now
            logger.info("Proxy pool refreshed: %d proxies available", len(_proxy_pool))
        else:
